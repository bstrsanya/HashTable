...
void LoadingData (Table* table)
{
    ...
    while (table->num[i])
    {
        int index = MurmurHash2 (table->num[i], strlen (table->num[i]));
...
// 1. Исследование хеш функций 2. Оптимизации (тики) 3. Кеш промахи
// inline
// volatale
// unlikely для бранч предиктора
// с какой функцией мерить
...
int crc32HashFunc (char *buf)
{
    ...
    while (buf[n])
    {
      crc = (crc << 8) ^ crc32_table[((crc >> 24) ^ buf[n]) & 255];
      n++;
    }
