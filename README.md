...
Для такой реализации необходимо чтобы каждый список был выровнен по 32-байтовой границе, иначе будет segmentation fault.
...
#### Результаты
Удалось увеличить скорость выполнения на $53,7\%$ относительно предыдущего запуска.

### Запуск 3 (выравнивание)
 
В отчетах perf видно что вызывается функция **__memmove_avx_unaligned_erms**, которая позволяет копировать данные несмотря на выравнивание. 
...
__m256i vec = _mm256_maskload_epi32 ((const int*) value, mask);
```
#### Отчет профилировщика
<img src="img/10.png" width="85%" align="center">

Добились того, что функция **__memmove_avx_unaligned_erms** не вызывается, но общее время не изменилось, поэтому нет смысла делать выравнивание в этом случае. Следовательно выравнивание бесполезно в этом месте, функция **_mm256_loadu_si256** работает быстро. Необходимо только выравнивание списков для хранения строк в формате **__m256i**.
...
#### Результаты
Удалось увеличить скорость выполнения на $28,27\%$ относительно $2$ запуска.
...
Eще раз попробуем оптимизировать вычисление хеша без циклов ассемблерной вставкой. Результат такой же как и в $5$ тесте. Сделано ради интереса сравнить два варианта одного кода.
```
uint8_t value[32] = {};
